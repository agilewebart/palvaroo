...
router.post('/v1/loginuser', async (req, res) => {
    try {
        ...
        if (finlRes.bkendFlag == 1) {
            return res.status(200).json({ success: finlRes.fendStuct.success, status: finlRes.fendStuct.status, message: finlRes.fendStuct.message, response: finlRes.fendStuct.response })
        }
        return res.status(200).json({ success: finlRes.fendStuct.success, status: finlRes.fendStuct.status, message: finlRes.fendStuct.message, response: finlRes.fendStuct.response })
    }
    catch (err) {
